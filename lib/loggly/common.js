...
//These are grouped just to handle commonly by displaying a message & no special action is needed.
var groupedHttpCodes = [httpStatusCode.notFound.code, httpStatusCode.badGateway.code, httpStatusCode.serviceUnavailable.code, httpStatusCode.gatewayTimeout.code]
...
common.loggly = function () {
  ...
  function handleRequestError(input, err) {
    if(err.response && groupedHttpCodes.indexOf(err.response.status) != -1) {
